...
var $author$project$Main$Model = F3(
	function (user, bookmarks, infomation) {
		return {bookmarks: bookmarks, infomation: infomation, user: user};
	});
var $elm$json$Json$Decode$decodeString = _Json_runOnString;
var $author$project$Main$Flags = F2(
	function (locationUrl, endpointBasePath) {
		return {endpointBasePath: endpointBasePath, locationUrl: locationUrl};
	});
var $author$project$Main$expectFlags = function (flagsDecodeResult) {
	if (flagsDecodeResult.$ === 'Ok') {
		var value = flagsDecodeResult.a;
		return value;
	} else {
		return A2($author$project$Main$Flags, 'http://localhost:8080', 'localhost:8080');
	}
};
...
var $author$project$Main$MultipleUser = {$: 'MultipleUser'};
var $author$project$Main$NothingUser = {$: 'NothingUser'};
var $author$project$Main$OnceUser = function (a) {
	return {$: 'OnceUser', a: a};
};
...
var $author$project$Main$extractUserFromParseResult = function (parseResult) {
	if (parseResult.$ === 'Nothing') {
		return $author$project$Main$NothingUser;
	} else {
		var userResult = parseResult.a;
		var _v1 = $elm$core$List$length(userResult);
		switch (_v1) {
			case 0:
				return $author$project$Main$NothingUser;
			case 1:
				return $author$project$Main$OnceUser(
					A2(
						$elm$core$Maybe$withDefault,
						'',
						$elm$core$List$head(userResult)));
			default:
				var _default = _v1;
				return $author$project$Main$MultipleUser;
		}
	}
};
var $author$project$Main$normalizePath = function (url) {
	if (url.$ === 'Nothing') {
		return $elm$core$Maybe$Nothing;
	} else {
		var e = url.a;
...
var $author$project$Main$isNotEmpty = function (string) {
	return !$elm$core$String$isEmpty(string);
};
var $author$project$Main$queryParser = A2(
	$elm$url$Url$Parser$Query$custom,
	'user',
	$elm$core$List$filter($author$project$Main$isNotEmpty));
var $author$project$Main$extractUserFromUrlString = function (url) {
	return $author$project$Main$extractUserFromParseResult(
		A2(
			$elm$core$Maybe$andThen,
			$elm$url$Url$Parser$parse(
				$elm$url$Url$Parser$query($author$project$Main$queryParser)),
			$author$project$Main$normalizePath(
...
var $author$project$Main$flagsDecoder = A3(
	...
	A2($elm$json$Json$Decode$field, 'locationUrl', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'endpointBasePath', $elm$json$Json$Decode$string));
var $author$project$Main$GotBookmarks = function (a) {
	return {$: 'GotBookmarks', a: a};
};
var $author$project$Api$Data$Bookmark = F2(
	function (url, title) {
...
var $author$project$Api$Data$bookmarkDecoder = A4(
	...
	A3(
		...
		'url',
		$elm$json$Json$Decode$string,
		$elm$json$Json$Decode$succeed($author$project$Api$Data$Bookmark)));
...
var $author$project$Api$Request$Bookmarks$getBookmarks = function (user_path) {
	return A7(
		...
		_List_fromArray(
			[
				_Utils_Tuple2(
				'user',
				$elm$core$Basics$identity(user_path))
			]),
		_List_Nil,
		_List_Nil,
		$elm$core$Maybe$Nothing,
...
var $author$project$Main$getBookmarks = F2(
	function (user, endpointBasePath) {
		return A2(
			$author$project$Api$send,
			$author$project$Main$GotBookmarks,
			A2(
				$author$project$Api$withBasePath,
				endpointBasePath,
				$author$project$Api$Request$Bookmarks$getBookmarks(user)));
	});
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Main$getBookmarksIfPresentsUser = F2(
	function (user, endpointBasePath) {
		if (user.$ === 'OnceUser') {
			var name = user.a;
			return A2($author$project$Main$getBookmarks, name, endpointBasePath);
		} else {
			var _default = user;
			return $elm$core$Platform$Cmd$none;
		}
	});
var $author$project$Main$infomation_loading = 'ロード中...';
var $author$project$Main$init = function (rawFlags) {
	var flags = $author$project$Main$expectFlags(
		A2($elm$json$Json$Decode$decodeString, $author$project$Main$flagsDecoder, rawFlags));
	var user = $author$project$Main$extractUserFromUrlString(flags.locationUrl);
	return _Utils_Tuple2(
		A3($author$project$Main$Model, user, _List_Nil, $author$project$Main$infomation_loading),
		A2($author$project$Main$getBookmarksIfPresentsUser, user, flags.endpointBasePath));
};
	...
	function (msg, model) {
		...
		if (result.$ === 'Ok') {
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{bookmarks: bookmarks, infomation: ''}),
			...
			switch (error.$) {
				case 'BadUrl':
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{infomation: 'ブックマークリストファイルのダウンロードに失敗しました。\nBadUrl: ' + url}),
						$elm$core$Platform$Cmd$none);
				case 'Timeout':
					return _Utils_Tuple2(
						_Utils_update(
...
var $author$project$Main$to_anchor = function (bookmark) {
	return A2(
		$elm$html$Html$li,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						A2($elm$core$Maybe$withDefault, bookmark.url, bookmark.title))
					]))
			]));
};
...
var $author$project$Main$view = function (model) {
	var infomation = function () {
		...
		switch (_v0.$) {
			case 'NothingUser':
				return 'ユーザーが指定されていません。クエリ文字列でユーザーを指定してください。\n例： user=mikoto2000';
			case 'MultipleUser':
				return 'ユーザーが複数指定されています。クエリ文字列でユーザーを指定してください。\n例： user=mikoto2000';
			default:
				var name = _v0.a;
				return model.infomation;
		}
	}();
	return A2(
		$elm$html$Html$div,
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$id('infomation')
							]),
						_List_fromArray(
							[
								$elm$html$Html$text(infomation)
							])),
						A2(
						$elm$html$Html$div,
						_List_fromArray(
							[
								$elm$html$Html$Attributes$id('bookmarks')
							]),
						_List_fromArray(
							[
								A2(
								$elm$html$Html$ul,
								_List_Nil,
								A2($elm$core$List$map, $author$project$Main$to_anchor, model.bookmarks))
							]))
					]))
			]));
};
